...
//     let vm = new Vue();
//     // 取消请求
//     if(vm.$axiosPromiseArr){
//         vm.$axiosPromiseArr.forEach((ele, index) => {
//             ele.cancel('001');
//             delete vm.$axiosPromiseArr[index];
//         })
//     }
//     next();
// })
...
Vue.prototype.$imgPhpUrl = process.env.VUE_APP_PHP_IMG_URL;
Vue.prototype.$phpUrl = process.env.VUE_APP_PHP_Url;
Vue.prototype.$simulateUrl = process.env.VUE_APP_Simulate_Url;
Vue.prototype.$interfaceGolangUrl = process.env.VUE_APP_GOLANG_apiUrl;
...
Vue.prototype.$loginRoute = function (route, type) {
    if (route) {
        let Obj = this.$getCookie('userId')
        switch(type) {
            // 模拟盘
            case 'msl':
                    sessionStorage.setItem('nextRoute', '/home');
                break;
            default: 
