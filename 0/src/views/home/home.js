  ...
  methods: {
    ...
    async getBanner() {
      ...
      data.data.data = data.data.data.filter(item => {
        return this.mslFilter(item);
      })
      this.bannerList = data.data.data.map(item => {
        let userName = this.mslFilter(item);
        if(typeof userName === 'string') {
          return {
            url: item.url+'?demo_cg='+userName,
            img: imgUrl + item.img
          }
        }
        return {
    ...
    // 过滤模拟盘数据,经纪人
    mslFilter(item) {
      let mslReg = new RegExp('msl.coinlot.io');
      if (mslReg.test(item.url)) {
        let Obj =decodeURIComponent(this.$getCookie('userId'));
        if(Obj) {
          let userName = JSON.parse(Obj).phone;
          let isAgent = localStorage.getItem('isAgent');
          if(isAgent === '1' && mslReg.test(item.url)) return userName;
        }
        return false
      }
      return true;
    ...
    gotoAddSei(){
      this.$router.push('/setting/community');
    },
    gotoAllData(data){
      let type=0;
      // 涨幅榜
