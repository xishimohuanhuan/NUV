					...
					<div class="from">
						<ul>
							<li>
								...
								<input @focus="isoldUser=false" type="number" v-model="phone" :placeholder="$t('invitauser.placephone')">
							</li>
							<li v-if="false">
							...
							<li>
								...
								<input type="text" v-model="invit" readonly :placeholder="$t('invitauser.yaoqingma')">
							</li>
						</ul>
						<div class="btn">
							<a style="height:100%" 
						...
						<div class="go-login"><span @click="goSign">{{$t('invitauser.gologin')}}</span></div>
					</div>
				</div>
			</li>
...
<script>
import {sendMobileCode, registByCode_api} from '../../api'
...
export default {
	...
	created() {
		...
		if(query.invit){
			...
			invit = localStorage.getItem('invit');
		}
		this.invit = invit;
	},
	methods:{
		endHandle(res) {
			this.sendmsgPost(this.idValue, res);
			this.countdown();
		},
		// 登录按钮方法
		LoginHandle() {
			// 是老用户提示登录
			if(this.isoldUser) {
				this.$normalToast(this.$t('otherWT.accUsedGLi'),2000);
				return;
			}
			...
			this.loginPostHandle().then(res =>{
				if(res.data.status === 200) {
					this.$normalToast(this.$t('register.registSuccess'),1000);
				...
				this.$normalToast(this.$t('otherWT.regLOPlaRe'),2000);
		...
		loginPostHandle() {
			...
			return registByCode_api(data);
		},
		// 去登录
		goSign() {
			this.$router.push('/invitaUser/invitaLogin')
		},
		// ios复制验证码会复制两次
		handleVerify() {
			...
			.then(res => {
				if(res.status === 200){
					...
					if(res.data.code === '001'){
						this.$normalToast(this.$t('otherWT.accUsedGLi'),1000);
						this.isoldUser = true;
						this.timemsg=0;
					}else {
						this.$normalToast(res.data.msg,1000);
