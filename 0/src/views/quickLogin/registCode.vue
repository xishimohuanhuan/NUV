...
export default {
    ...
    beforeRouteEnter(to, from, next) {
        next(vm => {
            ...
            vm.dataInit(to);
            if((vm.viewType===0) || (vm.viewType===1)){
                vm.setTimetCode();
            }
        })
    },
    ...
    methods:{
        ...
        async endHandle(x) {
            ...
            if(this.viewType===0){
                let { mobile } = temp;
                let params = this.getPicMobileParams(mobile, x);
                this.ingRequest=true;
                let res = await sendMessagePicAjax(params);
                this.sendCodeHandle(res);
            }
            // 再次验证图形验证码
            if(this.viewType===1){
                let { email } = temp;
                let params = this.getPicEmailParams(email, x);
                this.ingRequest=true;
                let res = await sendEmailPic(params);
                this.sendCodeHandle(res);
            }
        },
