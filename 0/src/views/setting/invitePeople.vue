    ...
    <div class="invitPeo">
        <div class="invit-detail" >
            ...
            <List
                ...
                @load="onLoad"
            >
            <div class="invit-wrap" v-for="ite in child1" :key="ite.userid">
                <div :class="['invit-title',addclaev===ite.userid ? 'add'+addclaev :'']" :id="'id'+ite.userid" @click="invitNum(ite.userid)">
                    <div class="invit-hea">
                        <div class="invit-right">{{$t('setting.notInvitation.rankOne')}}</div>
                ...
                <ul v-show='child2[ite.userid]'>
                    ...
                    <li 
                        ...
                        v-for="(item,index) in child2[ite.userid]"
                        :key="index"
                    >
                        <div class="two-detail" >
                            ...
                            <div class="invit-bm1">
                                <div class="add-col">{{item.username|filtPhone}}</div>
                    ...
                    <div :class="ite.indexNUm ? 'two-most' :'two-most1'" v-show="finished2[ite.userid]">
                        <span @click="hanldMoreCli(ite.userid)">{{$t('otherWT.hanCliMoe')}}</span>
                        <img @click="hanldMoreCli(ite.userid)" src="../../assets/img/setting/loadMost.png" alt="">
                    </div>
                </ul>
            </div>
            </List>
...
export default {
    ...
    filters:{
        ...
        filtPhone(val){
            ...
            let foot=val.slice(da.length-3,da.length);
            da=hea+'****'+foot;
            return da;
        },
    },
    methods:{
        ...
        getDataOne(intodata){
            if(intodata){
                ...
                this.loading=true;
                getinvitmessage(data).then(res=>{
                    if(res.data.status===200){
                        res.data.data.child1.map(ite=>{
                            ite.indexNUm=false;
                            return ite;
                        })
                        this.child1=this.child1.concat(res.data.data.child1);
                        if(Math.ceil(res.data.data.count/this.pageNum1)===this.currentPage1){
                            this.finished=true;
                        }else{
                            this.currentPage1++;
                        }
        ...
        requsetdata(intodata){
            if(!this.nextNum[intodata]){
                this.nextNum[intodata]=1
            }
            let data={
                    userId:JSON.parse(decodeURIComponent(this.$getCookie('userId'))).userId,
                    childId:intodata,
                    currentPage:this.nextNum[intodata],
                    pageNum:this.pageNum,
                    coinname:'usdt'
                }
                getinvitchildt(data).then(res=>{
                    if(res.data.status===200 && res.data.data.list.length>0){
                        if(this.child2[intodata]){
                            this.child2[intodata]=this.child2[intodata].concat(res.data.data.list);
                        }else{
                            // let hewi=document.getElementsByClassName("add"+this.addclaev)[0].offsetTop;
                            // document.getElementsByClassName("setting-body")[0].scrollTo(0,hewi-30);
                            this.$set(this.child2,intodata,res.data.data.list);
                            clearTimeout(this.timeer21);
                        ...
                        if(Math.ceil(res.data.data.count/this.pageNum)===this.nextNum[intodata]){
                    ...
                    this.isShow12=true;
                    this.toastWord=this.$t('otherWT.errwerwer');
                })
        },
        // 二级加载更多
        hanldMoreCli(dat){
