<template>
  <div class="info-record">
    ...
    <p class="info-record-h" v-text="$t('assets.recharge.record.intoTitle')"></p>
      ...
      <ul>
        <li @click="handleGo(item)" class="recos-item" v-for="(item, index) in recordList" :key="index">
          <div class="recos-it-h">
            <p>
              <span v-text="$t('assets.recharge.record.into')"></span>
              <span v-text="item.name"></span>
            </p>
            <p>
              <span class="over-completed" v-text="$t('assets.outRecDetail.status.completed')"></span>
          ...
          <div class="recos-it-b">
            ...
            <div class="recos-it-b-it">
              ...
              <p v-text="Number(item.num).toFixed(8)"></p>
            </div>
            <!-- <div class="recos-it-b-it">
...
export default {
  ...
  methods: {
    handleGo(item) {
      let that = this;
      that.$router.push({path: '/assets/intoRecDetail', query: {obj: JSON.stringify(item)}});
    },
    handleInfo() {
      let that = this;
      that.$axios({
        ...
        method: 'get',
        params: {
          coin: that.currency.toLowerCase(),
          page: 1,
          size: 100,
