      ...
      <ul>
        ...
        <li>
          ...
          <p class="t-left title-fiexd-hight">
            <span v-show="value !== 2">按{{priceType === 2?'触发':'执行'}}价格将收益：<span :class="profitPrice>0?'green':'red'">{{profitPrice | numFilter}}</span> USDT</span>
          </p>
        </li>
        <li>
...
export default {
  ...
  methods: {
    ...
    alterPopup() {
      let type = this.tabs[this.value].id+1  //止盈 1  止损2
      if(type===1&&this.profitPrice<0){
        this.$normalToast('止盈价格过低',2000);
        return
      }else if(type===2&&this.profitPrice>0){
        this.$normalToast('止损价格过高',2000);
        return
      }

      // console.log(this.profitPrice,'555555',this.tabs[this.value].id+1)
      // return
      this.showToast = true;
    },
    confirmBtnHandle() {
