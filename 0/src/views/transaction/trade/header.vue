...
export default {
    data(){
      let that = this;
        ...
        coinTitle: '',
        collectId: '',
        addTabs1: [{name: this.$t('trade.transaction'),path:'/main' ,id: 'main'}],
        addTabs2: [{name: this.$t('trade.transaction'),path:'/general' ,id: 'general'}],
        languageList: {
          'zh-cn': 0,
          'zh-tw': 1,
          'en-us': 2
    ...
    methods:{
        ...
        goHistory() {
          ...
          let icon = this.$route.params.icon;
          this.$router.push(`/trade/${icon}/history`);
        },
        // 添加自选
        addCollectHandle() {
          if(!this.userId) {
        ...
        goklinHandle() {
          ...
          this.viewMarketDetail(this.coinTitle, this.$route.params.icon, true, 'trade')
        },
        viewMarketDetail(coinTitle, coinName, isContract, path) {
          let that = this,
          ...
          if (/Native_iOS/.test(ua)) {
            ...
            prompt("cg://web/klinepage?" + JSON.stringify({coinTitle:coinTitle, coinName:coinName, language: that.languageList[that.$i18n.locale], userId: userId, collectId: collectId, isCollected: !!collectId, isContract: false, sessionId: window.sessionStorage.getItem('sessionId')}),"");
            return;
          }
