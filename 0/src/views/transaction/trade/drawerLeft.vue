    ...
    <div v-show="value" class="popup" @touchstart.stop="" @touchmove.stop="" @click="closePopupHandle">        
        <div class="popup-body" @click.stop="">
            ...
            <div class="popup-body-content">
                <div v-if="false" class="content-top" @click="toGeneralHandle">
                ...
                <div class="content-usdt">
                    ...
                    <p @click="changeCoinTypeHandle('BTC')" :class="{chooseType: chooseType === 'BTC'}">BTC</p>
                    <p @click="changeCoinTypeHandle('ETH')" :class="{chooseType: chooseType === 'ETH'}">ETH</p>
                </div>
            ...
            <ul class="content-coin" v-if='(renderCoin.length !== 0)'>
                <li 
                    ...
                    <div class="coin-price">{{isNaN(String(item.new_price)) ? '--' : item.new_price|toNonExponential}}</div>
                </li>
            </ul>
...
export default {
    ...
    methods: {
        ...
        changeCoinHandle(item) {
            ...
            if(pathName === 'tradeentrust') {
                pathName = 'tradebuy'
            }
            this.closePopupHandle();
            localStorage.setItem('trade_symbol',JSON.stringify(icon));
            this.$router.push(`/trade/${icon}/${pathName}`);
            return ;
        },
        ...
        usdtNewHandle(frist){
            ...
            if(coinInfo) {
                ...
                this.coinList = coinInfo.assitsCoinArr.concat(coinInfo.mainCoinArr);
                this.coinTypeFilter();
            }
            getCoinInfoNew({})
            .then(res => {
                if(res.status === 200) {
                    ...
                    this.coinList = res.data.assitsCoinArr.concat(res.data.mainCoinArr);
                    this.coinTypeFilter();
                }
            }).catch(() => {})
        },
