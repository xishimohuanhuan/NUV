<template>
	<div class="slider" v-show='isShowS'>
		<div 
			:class="['slider-model',model ? '':'slider-model1']"
			@click="modelHandle"
		></div>
		<div v-show="sliderShow" class="slider-box">
			<div class="slider-box-top">
				<img class="slider-box-bg" ref="bgimage" :src='renderData.background ? "data:image/png;base64,"+renderData.background :""' alt="">
				...
				<img 
					...
					:src='renderData.slider ? "data:image/png;base64,"+renderData.slider : ""' alt="">
			</div>
			...
			<div class="sliderContainer">
				...
				>
					<div 
						class="slider1"
						@touchmove.prevent='moveHandle'
						@touchstart.prevent='startHandle'
						@touchend.prevent='endHandle'
						ref='slideimg'
						:style='{left: left1+"px"}'
					>
				...
				<span class="sliderText" :style="{color: successColor?'red':''}">{{successColor?'请重新向右滑动填充拼图':'向右滑动填充拼图'}}</span>
			</div>
			<!-- 刷新 取消 -->
			<div class="end-re">
				<img 
...
export default {
    ...
    refresh(id) {
		...
		getImageData() {
			...
			formDate.append('id', this.idValue);
			this.$store.commit('changeLoading', true);
			this.$axios({
				url: '/user/sys/getImages',
				method: 'post',
				data: formDate,
				withCredentials: false
			}).then(res => {
				this.$store.commit('changeLoading', false);
				if(res.data.code === '001' && res.data.data.slider) {
					this.renderData = res.data.data; 
					this.sliderShow = true;
				} else {
					this.isShowS = false,
					this.$normalToast('滑块验证码加载失败',1000);  
				}
			}).catch(() => {
				this.$store.commit('changeLoading', false);  
				this.isShowS = false,
