        ...
        <div class="main">
            <div class="header-mas">
                <div class="h-num">
                    ...
                    <div @click="gotoPeople">{{teamCount}}</div>
                </div>
                <div class="h-man">
                    <div class="h-deng">
                    ...
                    <div class="h-goman" 
                        @click="gotoPeople" 
                    >{{'团队总人数'}}<span class="span-cosi"></span></div>
                </div>
                <div class="h-price">
                ...
                <div class="post-main">
                    ...
                    <div class="swiper-container">
                        <swiper :options="swiperOption" class="swiper-container" ref="mySwiper">
                            <swiper-slide class="swiper-slide d1" v-for="(ite,index) in powerRenderData" :key="index">
                                 <div class="power-swiper">
                                    <img :src="ite.image" class="main-img">
                                    <img :src="qrCodeImg" class="ewm-img">
                                </div>
                            </swiper-slide>
                        </swiper>
                        <div>
...
export default {
    ...
    beforeRouteEnter (to, from, next) {
        next(vm => {
            ...
            vm.userID = userId;
            getInvitationNum(userId).then(res=>{
                vm.invitNum=res.data.data;
                vm.getTwoCodeData(vm.invitNum);
            })
        });
    },
    ...
    methods:{
        ...
        getTwoCodeData(da){
            // 正式服
            // this.qrCodeImg = this.$interfaceJavaUrl+"/user/finance/qrCode?address=https://wap.coinlot.io/invitaUser/invitaSign?invit="+da;
            // 二维码
        ...
        //登陆获取用户的手机
        getuserinfo(){
            this.userinfo = JSON.parse(decodeURIComponent(this.$getCookie('userId')))
        },
        //跳转到佣金划转页面
        gotoPeopleTran(){
        ...
        // },
        showImg() {
            getInvitPoster({userId: this.userinfo.userId}).then(res => {
                if(res.data.status === 200) {
                    this.powerDataLoading = true;
                    this.$store.commit("changeLoading", false);
                    this.powerRenderData = [...res.data.data]
                }
            })
        },
        // 绘制背景banner
        drawBGImg(url, fn) {
            ...
            myImage.onload = function(){
                ...
                that.drawEWMImg(context,canvas, fn);
            }
        },
        // 绘制二维码在背景上
        drawEWMImg(context, canvas, fn) {
            let _this=this;
        ...
        shareHandle() {
            ...
            this.drawBGImg(this.powerRenderData[this.swiper.activeIndex].image, (baseImg) => {
                let newData= baseImg.slice(22);
                this.$store.commit("changeLoading", false);
                if (that.$clientEnv.isNativeiOS) {
                    that.$bridge.setupWebViewJavascriptBridge(function (bridge) {
        ...
        isShowSwiperHandle(){
            if((!this.powerDataLoading) || (!this.ewmImgLoaduing)) {
                this.$store.commit("changeLoading", true);
            } else {
                this.showPost=true;
            }
