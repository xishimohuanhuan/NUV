   ...
   data() {
    ...
    methods: {
        ...
        postFormHandle() {
            ...
            this.testmobile();
            if(!this.mobileRule1) return;
            let user = this.$getCookie('userId') && JSON.parse(decodeURIComponent(this.$getCookie('userId')));
            ...
            user.phone = this.mobile; //实名手机号更换
            postBindMobile(data)
            .then(res => {
                if(res.status === 200){
                    if(res.data.status === 200) {
                        this.$normalToast(`${res.data.msg},${this.$t('setting.towSecToSafe')}`);
                        this.$setCookie('userId', JSON.stringify(user), 1)  //登录成功
