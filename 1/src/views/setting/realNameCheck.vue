...
export default {
    ...
    methods: {
        ...
        fileChange(e, name){
            ...
            if(file) {
                if (!window.FileReader) {
                    ...
                    this.compressorImg(file).then(res => {
                        this.showImg(res, name);
                    }).catch((err) => {
                        this.$normalToast('请选择正确的图片',1500);
                    })
                }
            }
        },
        ...
        face_id_cardCompare(mobileCheck) {
            ...
            let extension = file.name ? file.name.split('.')[file.name.split('.').length-1] : 'jpg';
