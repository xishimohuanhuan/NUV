...
<!-- 委托 -->
    <div class="entrust-wrap" style="paddingBottom:20%;">
        ...
        <div class="entrust-main">
            <div class="entrust-header">
                <div :class="['entrust-one',isCheck ? 'entrust-bl' : ''] " 
                    @click="hanldWT('open')"
                    v-text="$t('entrust.openentr')"
                ></div>
                <div :class="['entrust-one',isCheck ? '' : 'entrust-bl'] "
                    @click="hanldWT('ping')" 
                    v-text="$t('entrust.unwindentr')"
                ></div>
                <div class="entrust-right" v-if="!isLogin && entrustproshow">
                    <div 
                        class="entrust-cex" 
                        v-text="$t('entrust.allcancel')"
                        @click="onePingCang"
                    ></div>
                    <img 
                        v-if="showCancel"
                        @click="onePingCang" 
                        src="../../assets/img/transaction/cex.png" 
                        alt=""
                    >
                    <img 
                        v-if="!showCancel"
                        src="../../assets/img/transaction/feih.png" 
                        alt=""
                    >
                </div>
            </div>
                    ...
                    <div class="entrustD-title">
                        <div class="entrustD-mz">
                            <div :class="['mz-one',( item.action===4 || item.side===2) ?'add-clo':'']">
                                {{item.side ===1  ? $t('trading.rose') :$t('trading.fall')}}{{item.action===4 ? $t('entrust.bigMove'):''}}
                            </div>
        ...
        <!-- 加载更多 -->
        <!-- <div 
            v-if='!isLogin && bastAllMach'
            class="allGengduo"
            @click="pushDataLast"
        >加载更多</div>
        <div class="foot-oding" v-show="!isLogin">
            <span v-show="loadingShow">加载中....</span>
            <span v-show="!bastAllMach && isTopBo">到底了！</span>
        </div> -->
        <!-- 未登录显示的页面 -->
...
export default {
    ...
    methods:{
        ...
        goOrder(ind){
            this.$router.push({
                path: '/transaction/'+this.dataToOrder[ind].symbol+'/order',
        ...
        getDataOrders(dataQ){
            if(this.$route.params.icon==="all"){
                ...
                getEntrustALLData(allData).then(res=>{
                    this.loading = false;
                    if(res.data.status===200 && res.data.data.list && res.data.data.list.length>0){
                        if(res.data.data.list.length===this.size){
                            this.page=this.page+1;
                            this.bastAllMach=true;
                            this.loadfn && this.loadfn()
                            // this.isTopBo=false;
                        }else if(res.data.data.list.length<this.size){
                            this.finished=true;
                            // this.isTopBo=false;
                    ...
                    }else if(res.data.status===200 && res.data.data.list===null){
                        this.enableLoadMore = false;
                    }else{
                        this.enableLoadMore = false;
                        this.loading = false; 
        ...
        processingData(data,daa){
            ...
            if(daa==='open'){
                let dataF=data.filter(item=>{
                    return item.action===0 && (item.state===0 ||item.state===3) && (item.amount !==item.filled_amount);
                })
                // console.log(dataF)
                //判断请求回来的数据是否小于10条
                ...
                //平仓委托
                let dataF=data.filter(item=>{
                    return (item.action===1 || item.action===4) && (item.state===0 || item.state===3);
                })
                //判断请求回来的数据是否小于10条
                if(dataF.length<6 && this.dataNewMore.length<6&&this.bastAllMach===true){
                    this.dataNewMore=this.dataNewMore.concat(dataF)
        ...
        dataToEnd(data){
            ...
            for(let i=0;i<data.length;i++){
                ...
                let jubuData={
                    ...
                    //交易方向买涨
                    side:this.handleCLick==='ping'?(data[i].side===1? 2:1):(data[i].side),
                    //交易方式,限价，市价
