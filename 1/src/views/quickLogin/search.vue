<template>
    <div id="search">
        <headers :title="$t('otherWT.conOrLo')" @back="$goBack"></headers>
        <div class="positionF">
            <img class="search" src="../../assets/img/quicklogin/search.png" alt="">
            <input 
                class="input" 
                v-model="text" 
                @input="searchName" 
                type="text" 
                :placeholder="$t('otherWT.SconOrLoN')"
            >
        </div>
        <div class="radiou">
            <div 
            ...
            >
                ...
                <span v-if="index==item.short" class="is-active" ></span>
            </div>
        </div>
    </div>
</template>
    ...
    export default {
        ...
        methods: {
            searchName(){
                this.searchArry=[]
                this.list.map(item=>{
                    if(item.name.toLocaleLowerCase().indexOf(this.text.toLocaleLowerCase())>-1 || item.en.toLocaleLowerCase().indexOf(this.text.toLocaleLowerCase())>-1){
                        this.searchArry.push(item)
                    }
                })
            },
            changeRadiou(e,obj) {
                this.index=obj.short
                this.$store.commit('countryItems', obj);
                window.history.go(-1)
            },
            classMethods(e){
                return `row ${e}`
    ...
    #search {
        ...
        .radiou {
            .is-active{
                ...
                background: red;
                float: right;
                background: url("../../assets/img/quicklogin/change.png") no-repeat;
